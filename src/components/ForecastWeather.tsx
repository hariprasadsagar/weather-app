...
import { CloudSun, AlertTriangle, Loader2 } from "lucide-react";
      ...
      {data?.forecast && (
        <div className="glass rounded-3xl p-5 space-y-3">
          {Object.entries(data.forecast).map(([dateKey, dayData]: [string, any]) => (
            <div key={dateKey} className="glass-subtle rounded-xl p-3">
              <p className="text-foreground font-medium text-sm">{dateKey}</p>
              <p className="text-muted-foreground text-xs">
                Min: {dayData.mintemp}°C | Max: {dayData.maxtemp}°C
              </p>
            </div>
          ))}
        </div>
